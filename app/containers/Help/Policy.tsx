...
export default function Policy() : JSX.Element{
  ...
  const langMenuClose = (changeLang) => {
    ...
    if(changeLang && changeLang!=lang){
      ...
      readAsset(`./assets/Htmls/${langVal.policyLink[changeLang]}`,(result) => {
        setContent(result);
      });
    }
  };
  ...
  useEffect(()=>{
    ...
    if(langVal && Object.keys(langVal).length>0){
      readAsset(`./assets/Htmls/${langVal.policyLink[lang]}`,(result) => {
    ...
    <DetectIdle>
      <div className={classes.bgBlack}>
        ...
        <div className={`text-white ${classes.content} ${classes.wrapper} ${classes.bgBlack}`}>
          <h1 className="text-center text-uppercase">privacy policy</h1>
