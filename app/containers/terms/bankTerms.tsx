...
export default function BankTerms() : JSX.Element{
  ...
  useEffect(()=>{
    ...
    if(langVal && Object.keys(langVal).length>0){
      readAsset(`./assets/Htmls/${langVal.bankTerm[lang]}`,(result) => {
      ...
      <div className={classes.bgBlack}>
        ...
        <div className={`text-uppercase d-flex justify-content-between align-items-center ${classes.bgBlack} ${classes.footer}`}>
          ...
          <div>
            ...
            <Button color="success" className={`${classes.font24} mr-3`} href={`#${routes.BankOpt}`} > {langVal && langVal.acceptTerm? langVal.acceptTerm[lang] : ""}</Button>
          </div>
        </div>
      </div>
    </DetectIdle>
